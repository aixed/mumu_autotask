...
Frida 在已登录的游戏进程内调用原生 Lua 业务入口，复用游戏自己的连接和鉴权状态。

Lua 主状态由 ADB 只读扫描 `/proc/<pid>/maps` 与 `/proc/<pid>/mem` 定位；Lua
只会通过 Unity 的 `queueGLThreadEvent(FutureTask)` 在 `UnityMain` 线程执行。流程
不会发送 HOME、不会后台化游戏，也不会安装 Lua inline hook。
...
- 目标绑定运行 ID、任务 ID、品质、坐标、过期时间、怪物 ID、等级和体力消耗；
  任一字段变化都会停止。
- 真正出征只调用游戏按钮链 `OnBtnAverageClick()` -> `OnBtnGoOnClick()`，不绕过
  游戏自身的队列、体力、编队和特殊战争检查。

| ADB serial | MuMu 实例 | 允许角色 | 本机 Frida 地址 |
| --- | --- | --- | --- |
| `127.0.0.1:16384` | `MuMuPlayer-12.0-0` | `打工人`, `打工魂` | `127.0.0.1:27042` |
| `127.0.0.1:16416` | `MuMuPlayer-12.0-1` | `打工的` | `127.0.0.1:27052` |
| `127.0.0.1:16480` | `MuMuPlayer-12.0-3` | `打工客`, `打工仔` | `127.0.0.1:27062` |
...
3. 点击“刷新情报”读取当前角色和可用骷髅目标。
4. 复选绿色、蓝色、紫色和黄色中的一个或多个品质。
5. 用“同时出征”滑块选择每波 `1-3` 队。
6. 点击“情报-自动狩猎野兽”，不再弹出二次确认，程序立即按当前设备、品质和并发数开始执行。
...
冻结的并发数切成波次。一波内会先同时提交全部精确目标的“平均配置 -> 出征”任务，
并独立核对每个回执；
只有该波每个精确 runtime ID 都由只读状态轮询证明为 `COMPLETED/MISSING` 后，才会
启动下一波。任一波等待失败时会先做一次只读角色和情报核对，然后立即停止批次，
把尚未发起的后续波次标记为“跳过”，并禁止领取，避免在状态不确定时继续出征。
...
CLI 在一个 Frida 会话中依次完成 INTEL、OPEN、READY、COMMIT、VERIFY。VERIFY
