...
elseif slot2 then
	...
	if uv1.CheckAllianceSafeResource(slot0.marchType) and not GCtrl.AllianceCtrl:IsHadAllaince() then
		uv3.OpenToast(i18n("common_tips_232"))

		return
	end

	if slot0.allStamina < slot0:GetCostStaminaEduce(slot3, slot0.showHeroList) then
		uv3.OpenGetResource({
			{
				count = 1,
	...
	slot6, slot7 = uv1.IsBeforehandMarch(slot0.marchMapType, slot0.marchType, slot0.mapObjType, slot0:GetExtra(), true)
