...
class BusinessTests(unittest.TestCase):
    ...
    def test_world_monster_level_and_native_call_chain(self) -> None:
        ...
        self.assertIn("GViewId.WORLD_SEARCH_OBJ", search)
        self.assertIn("local view_id = 207", search)
        self.assertIn("view.OnBtnSearchClick", search)
        self.assertIn("view._selectedObjType = 1", search)
        self.assertIn("view.objLvMap[1] = LEVEL", search)
        self.assertNotIn("ReqWorldMapSearch", search)
        self.assertNotIn("SearchToMapObj", search)
        self.assertIn("helper.SearchToMapObj", search_result)
        self.assertIn("state.world_x, state.world_y, kingdom, 1, 1, true", search_result)
        self.assertIn("state.map_callback, false", search_result)
