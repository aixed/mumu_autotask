...
`;
  ...
  // tools/frida_java_probe.js
  rpc.exports = {
    inspect() {
      if (!frida_java_bridge_default.available) {
        return { available: false, classes: [] };
      }
      return frida_java_bridge_default.performNow(() => {
        ...
        const names = frida_java_bridge_default.enumerateLoadedClassesSync().filter((name) => selected.has(name)).sort();
        const classes = [];
        for (const name of names) {
          try {
            const wrapper = frida_java_bridge_default.use(name);
            classes.push({
              name,
              methods: wrapper.class.getDeclaredMethods().map((method) => method.toString()).sort(),
              fields: wrapper.class.getDeclaredFields().map((field) => field.toString()).sort()
            });
          } catch (error) {
